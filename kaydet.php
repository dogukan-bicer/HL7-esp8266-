...
<html>
    ...
    <body>
            ...
            if ($_FILES['upload_file1']['size'] <= 0) {
                ...
                foreach ($_FILES as $key => $value) {
                    if (0 < $value['error']) {
						...
						$dbConn = mysqli_connect('localhost', 'root', '123456789','hl7_server') or die('MySQL connect failed. ' . mysqli_connect_error());
						
						$sql = "insert into hl7_kayit(name, type, size, content, saved_date) values('".$value['name']."', '".$value['type']."', '".filesize_formatted($value['size'])."', '".mysqli_escape_string($dbConn, file_get_contents($value['tmp_name']))."', '".date('Y-m-d H:i:s')."')";
        ...
        ?>
        <div id="container">
            <form name="upload_form" enctype="multipart/form-data" action="kaydet.php" method="POST">
                <fieldset>
